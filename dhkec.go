...
var (
	// first 6 bits are 000101
	DHKECGenesisMarker = NewBitList(5, 6)
	// first 6 bits are 001010
	DHKECTransferMarker = NewBitList(10, 6)
)

func init() {
	RegisterColorKernel(&DHKEC{MinimumSatoshi: int64(5430)})
}
...
func (k DHKEC) fetchPadding(tx *btcwire.MsgTx) int64 {
...
func (k DHKEC) Code() string {
	return "DHKEC"
}

func (k DHKEC) paddingNeeded(cv ColorValue) (BitList, int64) {
...
func (k DHKEC) computeSequence(m1, m2 BitList) uint32 {
...
func (k DHKEC) IssuingSatoshiNeeded(cv ColorValue) int64 {
	_, padding := k.paddingNeeded(cv)
	return padding + int64(cv)
}

func (k DHKEC) getChange(b *BlockExplorer, inputs []*btcwire.OutPoint, outputs []*ColorOut, fee int64) (*int64, error) {
	sum := int64(0)
...
func (k DHKEC) OutPointToColorIn(b *BlockExplorer,
...
func (k DHKEC) ColorInsValid(b *BlockExplorer, genesis *btcwire.OutPoint,
...
func (k DHKEC) IssuingTx(b *BlockExplorer, inputs []*btcwire.OutPoint,
	...
	sequence := k.computeSequence(DHKECGenesisMarker, exponentMarker)
...
func (k DHKEC) TransferringTx(b *BlockExplorer, inputs []*ColorIn,
	outputs []*ColorOut, changeScript []byte,
	...
	sequence := k.computeSequence(DHKECTransferMarker, exponentMarker)
	msgTx := btcwire.NewMsgTx()
	for i, input := range inputs {
...
func (k DHKEC) CalculateOutColorValues(genesis *btcwire.OutPoint, tx *btcwire.MsgTx, inputs []ColorValue) ([]ColorValue, error) {
...
func (k DHKEC) txColorIns(b *BlockExplorer, tx *btcwire.MsgTx) ([]*ColorIn, error) {
...
func (k DHKEC) AffectingIndexes(colorIns []*ColorIn, outValues []int64, padding int64, outputIndexes []int) ([]int, error) {
...
func (k DHKEC) FindAffectingInputs(b *BlockExplorer, genesis *btcwire.OutPoint, tx *btcwire.MsgTx, outputIndexes []int) ([]*btcwire.OutPoint, error) {
